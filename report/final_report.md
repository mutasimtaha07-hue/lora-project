...
result JSONs — not regenerated for this report) are embedded here alongside the tables
above.

![Accuracy vs. LoRA Rank](../figures/accuracy_vs_rank.png)

*Mean accuracy vs. LoRA rank, both datasets, with std error bars.*

![F1 vs. LoRA Rank](../figures/f1_vs_rank.png)

*Mean F1 vs. LoRA rank, both datasets, with std error bars.*

![LoRA vs. Full Fine-Tuning](../figures/lora_vs_full_ft.png)

*Grouped bar chart comparing all five LoRA ranks against the (visually distinct, hatched)
full-fine-tuning reference bars, explicitly labeled as single-seed with no error bar.*

![Trainable Parameters vs. Rank](../figures/trainable_params_vs_rank.png)

*Trainable parameter count vs. rank, log-scaled Y-axis, with the full-fine-tuning
parameter count shown as a horizontal reference line roughly two orders of magnitude
above the LoRA curve.*

![Parameter Efficiency: Accuracy vs. % of Full-FT Parameters](../figures/parameter_efficiency.png)

*Mean accuracy vs. trainable parameters (as % of full fine-tuning, log-scaled X-axis),
with full-fine-tuning reference points at 100%.*

![Rank Sensitivity: Accuracy Change Relative to r=1](../figures/rank_sensitivity.png)

*Change in mean accuracy relative to r=1, for both datasets, on a deliberately narrow
Y-axis range to make the small magnitude of the rank effect visually obvious.*
